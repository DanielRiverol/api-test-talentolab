...
router.get("/json/:id", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Error al obtener usuario", err: err.message });
  }
});
...
router.get("/xml/:id", async (req, res) => {
  try {
    ...
    if (!usuario) {
      return res.status(404).set("Content-Type", "application/xml").send("<error>Usuario no encontrado</error>");
    }
    ...
    res.status(500).set("Content-Type", "application/xml").send(`<error>Error al obtener usuario: ${err.message}</error>`);
  }
});
...
router.delete("/json/:id", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Error al eliminar usuario", err: err.message });
  }
});
...
router.delete("/xml/:id", async (req, res) => {
  try {
    const usuario = await Usuario.findByIdAndDelete(req.params.id);
    if (!usuario) {
      return res.status(404).set("Content-Type", "application/xml").send("<error>Usuario no encontrado para eliminar</error>");
    }
    const xml = builder.buildObject({ mensaje: "Usuario eliminado correctamente" });
    res.set("Content-Type", "application/xml");
    res.status(200).send(xml);
  } catch (err) {
    res.status(500).set("Content-Type", "application/xml").send(`<error>Error al eliminar usuario: ${err.message}</error>`);
  }
});
/* PUT JSON */
router.put("/json/:id", upload.single("cv"), async (req, res) => {
  try {
    ...
    if (cv !== undefined) { // Solo añadimos el CV si se envió uno nuevo
    ...
    res.status(500).json({ error: "Error al actualizar usuario", err: err.message });
  }
});
...
router.put("/xml/:id", upload.single("cv"), async (req, res) => {
  try {
    ...
    if (!usuarioActualizado) {
      return res.status(404).set("Content-Type", "application/xml").send("<error>Usuario no encontrado para actualizar</error>");
    }
