## 🧪 Pruebas GET (Obtener todos)

Este script comprueba la respuesta de un endpoint que devuelve una lista de usuarios. Es robusto y puede manejar tres tipos de estructuras de respuesta comunes:

1.  Un array directo: `[ ... ]`
2.  Un objeto anidado: `{ "usuarios": [ ... ] }`
3.  Un objeto anidado genérico: `{ "data": [ ... ] }`
...
Estas pruebas validan la respuesta al solicitar un recurso específico. Utiliza una variable de entorno `{{userID}}` para comprobar que se devolvió el usuario correcto.
...
## ➕ Pruebas POST (Crear Recurso)

Estas pruebas verifican la creación de un nuevo recurso. El código también incluye una sección crucial para **guardar el `_id` del nuevo recurso** en una variable de entorno (`userID`), permitiendo que las pruebas de PUT y DELETE lo utilicen.
...
// 3. Prueba: La respuesta contiene los datos esperados
//    Asumiendo que la API devuelve un mensaje y el objeto creado.
pm.test("La respuesta incluye 'mensaje' de confirmación y 'usuario'", function () {
    // Convertimos la respuesta JSON en un objeto de JavaScript
    const jsonData = pm.response.json();
    
    // Ajusta esto a tu respuesta real (ej: "mensaje" y "usuario")
    pm.expect(jsonData).to.have.property("mensaje");
    // Ejemplo del script original
    pm.expect(jsonData).to.have.property("usuario"); // Ejemplo basado en el script de guardar ID
});
...
## 🔄 Pruebas PUT (Actualizar Recurso)

Estas pruebas validan la actualización de un recurso existente, usando el `userID` guardado en el paso anterior. Comprueba que el mensaje de éxito sea correcto y que los datos devueltos reflejen la actualización.
...
## ❌ Pruebas DELETE (Eliminar Recurso)

Finalmente, estas pruebas confirman que el recurso se elimina correctamente y, como buena práctica, limpian la variable de entorno `userID` para evitar su reutilización accidental.
